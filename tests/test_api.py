...
def test_create_bookmark_missing_fields(client):
    ...
    # Missing name
    response = client.post("/api/bookmarks", json={"url": "https://example.com"})
    assert response.status_code == 400

    # Missing URL
    response = client.post("/api/bookmarks", json={"name": "test"})
    assert response.status_code == 400
...
def test_create_bookmark_duplicate_name(client, sample_bookmark):
    ...
    response = client.post(
        "/api/bookmarks", json={"name": "test", "url": "https://example.com"}
    )
    assert response.status_code == 400
    data = response.get_json()
...
def test_update_bookmark(client, sample_bookmark):
    ...
    response = client.put(
        f"/api/bookmarks/{sample_bookmark.id}",
        json={
            "name": "updated",
...
def test_add_alias(client, sample_bookmark):
    ...
    response = client.post(
        f"/api/bookmarks/{sample_bookmark.id}/aliases", json={"alias": "newtest"}
    )
    assert response.status_code == 201
    data = response.get_json()
...
def test_add_alias_duplicate(client, sample_bookmark):
    ...
    response = client.post(
        f"/api/bookmarks/{sample_bookmark.id}/aliases",
        json={
