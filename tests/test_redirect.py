...
def test_redirect_special_characters(client, sample_bookmark):
    ...
    # Use URL encoding to pass special characters in the query
    response = client.get("/?q=test%20hello%26world")
    assert response.status_code == 302
    # Special characters should be URL encoded in the redirect location
    assert "%26" in response.location or "hello" in response.location
